...
struct PersistenceController {
  ...
  func addEntity(name: String, storagePath: String, localPath: String) throws {
    let newItem = FileMetadata(context: container.viewContext)
    newItem.name = name
    newItem.storagePath = storagePath
    newItem.localPath = localPath
  
    try container.viewContext.save()
  }
  
  func deleteEntity(for obj: NSManagedObject) throws {
    // Should be called after deleting the file from File system
